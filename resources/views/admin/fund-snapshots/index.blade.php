...
@section('content')
    ...
    <div class="notice notice-info admin-notice-top">
        <p>{{ __('The latest row is shown on the public Daily Fund Prices page. Every save here also updates that date in the matching month on') }} <a href="{{ route('admin.cms-tables.edit', ['tableKey' => 'fund_prices_archive']) }}">{{ __('Fund price archives') }}</a> {{ __('(e.g. June 2026 on fund-price-archive-2026).') }}</p>
    </div>
    ...
    <table class="wp-list-table widefat fixed striped">
        <thead>
            <tr>
                ...
                <th scope="col" class="admin-col-actions--140">{{ __('Actions') }}</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($snapshots as $s)
                <tr>
                    ...
                    <td>
                        ...
                        <form action="{{ route('admin.fund-snapshots.destroy', $s) }}" method="post" class="admin-form-inline" onsubmit='return confirm({{ json_encode(__('Delete this row?')) }});'>
                            @csrf
                            @method('DELETE')
