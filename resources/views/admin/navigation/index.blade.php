    ...
    <table class="wp-list-table widefat fixed striped">
        <thead>
            <tr>
                ...
                <th scope="col" class="admin-col-order">{{ __('Order') }}</th>
            ...
            @forelse ($items as $row)
                <tr>
                    ...
                    <td>
                        @switch($row->link_type)
                            ...
                            @case(\App\Models\NavMenuItem::LINK_CUSTOM_URL)
                                <span class="description admin-description-wrap">{{ \Illuminate\Support\Str::limit($row->custom_url, 60) }}</span>
                                @break
                    ...
                    <td>
                        ...
                        <form action="{{ route('admin.navigation.destroy', $row) }}" method="post" class="admin-form-inline" onsubmit='return confirm({{ json_encode(__('Delete this item and its children?')) }});'>
                            @csrf
                            @method('DELETE')
